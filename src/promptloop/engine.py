...
from .tokens import count_tokens, TokenTimer
...
def run_chat(
    ...
    *,
    ...
    if verbose:
        ...
        sentinel_instr = f" (Type '{auto_sentinel}' on new line to send)" if auto_sentinel else ""
    ...
    try:
        while True:
            ...
            # --- CUSTOM EXIT CHECK ---
            if not user_input or user_input.lower() in [k.lower() for k in exit_keywords]:
                break
            
            # Input Safety
            if len(user_input) > (max_input_tokens * 5):
                 emit(f"⚠️ Input too long (approx {len(user_input)//4} tokens). Limit is {max_input_tokens}.\n")
                 continue
            ...
            sampler = make_sampler(temp=final_temp, top_p=final_top_p, top_k=final_top_k)
            response_text = ""
            timer = TokenTimer()
            first_chunk = True
            ...
            try:
                if stream:
                    ...
                    for response in stream_generate(model, tokenizer, prompt=prompt, max_tokens=max_tokens, sampler=sampler):
                        if guardian: guardian.check()

                        if first_chunk:
                            if response_prefix: emit(response_prefix)
                            first_chunk = False
                    ...
                    if response_prefix: emit(response_prefix)
                    response_text = generate(model, tokenizer, prompt=prompt, max_tokens=max_tokens, sampler=sampler, verbose=False)
